...
public class SlidingWindowLogRateLimiter implements RateLimiter {
    ...
    public SlidingWindowLogRateLimiter(StringRedisTemplate redis, RateLimitProperties props) {
        this.redis = redis;
        this.props = props;
        this.script = new DefaultRedisScript<>();
        this.script.setScriptSource(
                new ResourceScriptSource(new ClassPathResource("scripts/sliding_window_log.lua")));
        this.script.setResultType(List.class);
    }

    @Override
    public RateLimitResult tryAcquire(String clientId) {
        String key = "rl:sliding_log:" + clientId;
